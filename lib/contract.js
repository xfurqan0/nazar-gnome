...
//   4. A reading expires. A percentage measures a window, so once that window is over —
//      the tray called it `stale`, or its reset has passed with no tray running to correct
//      it — the number is history and this file hands back nothing in its place. See
//      `panelReading`; the panel that sat on 67 % for four days after its weekly reset is why.
...
/**
 * Binary float residue, absorbed before rounding down.
 *
 * The contract stores percentages as reported, unrounded, and a writer that computed one as
 * `fraction * 100` hands over `56.99999999999999` for a true 57 and `14.000000000000002`
 * for a true 14 — no decimal fraction of the form `x / 100` is exactly representable in
 * binary. A bare floor turns that first number into 56, so the panel reads one point short
 * on exactly the values that land on an integer. The tolerance is far below any precision
 * this extension ever displays: a real 28.9999 still floors to 28, and 99.6 still floors
 * to 99.
 */
const PERCENT_NOISE = 1e-9;

/** Rule 5: down, never to the nearest. A window is not spent until it is spent. */
export function floorPercent(percent) {
    return percent === null ? null : Math.floor(percent + PERCENT_NOISE);
}
...
/**
 * The one number that answers "how full is this machine": the highest binding percentage of
 * any provider, floored — and only while that number can still be true. The GNOME face leads
 * its menu with it and draws its indicator from whether there is one at all.
 *
 ...
 *
 * The second half of this is the rule that a panel showing 67 % for four days taught: a
 * reading is a measurement of a window, and it stops being a measurement of anything when
 * the window it measured is over. Two shapes of that, and only two:
 *
 *   1. `state: "stale"`. The tray says so itself — it marks a window stale when the reset
 *      passed and nothing has been read since — and a face that draws the number anyway is
 *      contradicting the only program that knows.
 *   2. `resetsAt` is behind `now` **and the tray is not running**. Nothing is coming to
 *      correct it; the window that produced the number no longer exists.
 *
 * While the tray *is* running, an expired `resetsAt` is left alone. The tray re-reads within
 * the minute, and Claude Code's five-hour window renews from the first request of a new
 * session rather than on the clock, so a reset that has just passed with a live engine
 * behind it is a reading about to be replaced rather than a wrong one. Whether that
 * indulgence belongs to Claude alone is an open question in nazar-tray's own PROJECT.md, and
 * the day it is answered there this is the one function to change.
 *
 * `running` is `trayStatus().running`: `true`, `false`, or `null` where the caller cannot
 * tell. `null` keeps the number, for the same reason the tray's own rule does — "cannot
 * tell" is never "nobody is there", and blanking a real percentage on a machine whose
 * `/proc` could not be read would be inventing an outage.
 *
 * The believability test is applied to the *winning* window and nowhere else. Falling back
 * to some other provider's smaller number would answer a question nobody asked: if the
 * window closest to full cannot be trusted, the honest answer is that we do not know how
 * close to full this machine is, and no reading at all is that answer. Rule 2 again — never
 * a reassuring 0 %, and never a reassuring 5 % either.
 */
export function panelReading(doc, {now = Date.now(), running = null} = {}) {
    const empty = {percent: null, severity: 'unknown', provider: null, key: null, reason: null};
    ...
    for (const name of Object.keys(doc.providers).sort()) {
        ...
        if (best === null || percent > best.percent)
            best = {percent, provider: name, key: binding.key, window: binding.window};
    }
    if (best === null)
        return empty;

    const reason = expiryReason(best.window, {now, running});
    if (reason !== null)
        return {percent: null, severity: 'unknown', provider: best.provider, key: best.key, reason};

    return {
        percent: best.percent,
        severity: severity(best.percent),
...
/**
 * Why a window's percentage may no longer be shown as a fact, or `null` when it may.
 *
 * Short by design: the string is appended to a menu row that already says when the reset was
 * due, so it carries the cause and not the whole sentence.
 */
export function expiryReason(window, {now, running}) {
    if (!isObject(window))
        return null;

    if (window.state === 'stale')
        return 'the tray marked it stale';

    const resets = instant(window.resetsAt);
    if (running === false && resets !== null && resets <= now)
        return 'tray not running';
 ...
 */
export function providerViews(doc, {now, timeZone, running = null}) {
    if (!isObject(doc) || !isObject(doc.providers))
        return [];

    const binding = panelReading(doc, {now, running});
    return Object.keys(doc.providers).sort().map(name => {
        ...
        return {
            ...
            windows: windowRows(provider, {
                now,
                timeZone,
                binding: own?.key ?? null,
                panel: binding,
                providerName: name,
            }),
        };
 ...
 */
function windowRows(provider, {now, timeZone, binding, panel, providerName}) {
    if (!isObject(provider) || !isObject(provider.windows))
        return [];
    ...
    return keys.map(key => {
        ...
        // The row the panel is refusing to draw a number for. It keeps its own percentage —
        // a menu is where you go to find out what the `?` is hiding, and hiding it there too
        // would be a panel that says nothing twice — but the detail has to carry the cause,
        // because "reset was due 02:00 Sat" on its own is a fact and not an explanation. A
        // stale row is left alone: it already ends in `· stale`, and saying it twice is the
        // kind of noise that makes a menu unreadable.
        const refused = panel.reason !== null && panel.provider === providerName && key === panel.key;
        const detail = windowDetail(window, now, timeZone);
        return {
            key,
            ...
            panelBinding: panel.provider !== null && key === panel.key,
            panelRefused: refused,
            detail: refused && !stale && detail ? `${detail} · ${panel.reason}` : detail,
        };
    });
}
